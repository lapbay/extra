...
#define SinaWeiBoSDKDemo_APPKey @"2707380334"
#define SinaWeiBoSDKDemo_APPSecret @"cdbf41cb66cfdc56231906d7046629b6"
...
- (void)viewDidLoad {
	...
	[button setTitle:@"Feed to Voice" forState:UIControlStateNormal];
	[button addTarget:self action:@selector(getMyFeedButtonPressed:) forControlEvents:UIControlEventTouchUpInside];
	[self.view addSubview:button];
...
- (void)getMyFeedButtonPressed:(id)sender
{
	[weibo requestWithMethodName:@"statuses/home_timeline.json" andParams:nil andHttpMethod:@"GET" andDelegate:self];
}
...
- (void)request:(WBRequest *)request didLoad:(id)result
{
	...
	if ([urlString rangeOfString:@"statuses/public_timeline"].location !=  NSNotFound || [urlString rangeOfString:@"statuses/home_timeline"].location !=  NSNotFound)
	{
		alert.message = @"获取成功";
        NSString *weiboText;
        NSArray *publicWeibo = result;
        NSLog(@"%@",result);
        if (publicWeibo.count >= 1 ) {
            //weiboText = [[feed objectForKey:@"text"] retain];
            weiboText = @"";
            NSInteger count;
            if (publicWeibo.count >= 3) {
                count = 3;
            }else{
                count = publicWeibo.count;
            }
            for (int i = 0; i < count; i++) {
                NSDictionary *feed = [publicWeibo objectAtIndex:i];
                NSString *vText;
                if ([feed objectForKey:@"retweeted_status"]) {
                    vText = [[feed objectForKey:@"retweeted_status"] objectForKey:@"text"];
                }else{
                    vText = [feed objectForKey:@"text"];
                }
                NSLog(@"%i",vText.length);
                weiboText = [weiboText stringByAppendingFormat:@"%@\r\n\r\n",vText];
                [weiboText retain];
            }
        }else{
            weiboText = @"没有获取到新微博";
        }
        [_synthesizerController setDefaultText:weiboText];
        [_synthesizerController.tableView reloadData];
